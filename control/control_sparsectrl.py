...
from comfy.cldm.cldm import ControlNet as ControlNet_cldm
...
class SparseControlNet(ControlNet_cldm):
    def __init__(self, *args,**kwargs):
        ...
        use_simplified_conditioning_embedding = kwargs.get("use_simplified_conditioning_embedding", False)
        if use_simplified_conditioning_embedding:
            self.input_hint_block = TimestepEmbedSequential(
                    operations.conv_nd(self.dims, hint_channels, self.model_channels, 3, padding=1, dtype=self.dtype, device=device),
            )
    ...
    def forward(self, x, hint, timesteps, context, y=None, **kwargs):
        ...
        conditioning_mask1 = torch.ones_like(hint[:, :1])
        conditioning_mask2 = torch.zeros_like(hint[:, :1])
        conditioning_mask = conditioning_mask2
        conditioning_mask[0] = conditioning_mask1[0]
        conditioning_mask[16] = conditioning_mask1[16]
        #conditioning_mask[15] = conditioning_mask1[15]
        #conditioning_mask[31] = conditioning_mask1[31]
        modified_hint = torch.zeros_like(hint)
        modified_hint[0] = hint[0]
        modified_hint[16] = hint[16]
        #modified_hint[15] = hint[15]
        #modified_hint[31] = hint[31]
        hint = torch.cat([modified_hint, conditioning_mask], dim=1)
...
# main class for holding SparseControlNet
class SparseControlNetOld(nn.Module):
    def __init__(
        self,
        image_size,
        in_channels,
        ...
        **kwargs,
        ...
        #self._motion_wrapper: SparseCtrlMotionWrapper = None
